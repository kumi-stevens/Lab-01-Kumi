...
**there are value equal to na in the original data set**
...
**lots of small values, but big values are humungeous**
...
_How many tourist attractions are there in each of the 4 nations? From this, discuss in your group how reliable you think the inter-quartile estimates are. the ones with a higher total count will have a higher degree of accuray on their IQR._
...
TotalV <- visitors_with_nations %>%
  count(nation)
 
Costs <- visitors_with_nations %>% 
  filter(admission != "Free") %>% 
  count(nation)

Prop = 1- (select(Costs, n) / select(TotalV, n))
...
visitors_with_nations %>%
  mutate(
    PChange = ((n_2022 / n_2021)*100)
 ) %>% 
  filter(nation == "Scotland") %>% 
  group_by(admission) %>% 
  summarise(
     Mean_for_group = mean(PChange, na.rm = TRUE)
  ) %>% 
  arrange(Mean_for_group)
...
#4
filter(visitors, attraction== "National Museum of Scotland" )
#free
...
(visitors %>% 
  ...
  filter(admission == "Members") %>% 
  filter(n_2022 > 100000))

#3 - fountains abbey Estate, kirkstall abbey, RHS Garden Harlow carr 
